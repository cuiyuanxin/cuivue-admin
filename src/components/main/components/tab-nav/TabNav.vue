	...
	<div class="scroll-outer" ref="scrollOuter" @DOMMouseScroll="handlescroll" @mousewheel="handlescroll">
		<div ref="scrollBody" class="scroll-body" :style="{left: tagBodyLeft + 'px'}">
			<transition-group name="taglist-moving-animation">
				<Tab :class="item.name === value.name ? 'tab-selected' : ''" v-for="item in list" :key="`tab-nav-${item.name}`" :name="item.name" @on-close="handleClose" @on-select="handleSelect(item)" :closable="item.name !== 'home'" :icon="item.meta.icon">
...
export default {
	...
	methods: {
		...
		handleTabClick(type) {
			if (type === 'close-all') {
				// 关闭所有，除了home
				let res = this.list.filter(item => item.name === HOME_PAGE_NAME)
				this.$emit('on-close', res, 'all')
			} else {
				// 关闭除当前页和home页的其他页
				let res = this.list.filter(item => item.name === this.value.name || item.name === HOME_PAGE_NAME)
